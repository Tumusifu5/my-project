...
$receivers = getUsersByRole('receiver');
$selectedReceiver = isset($_GET['receiver_id']) ? (int)$_GET['receiver_id'] : null;

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message']) && $selectedReceiver) {
    $message = trim($_POST['message']);
    if (!empty($message)) {
        sendMessage($_SESSION['user_id'], $selectedReceiver, $message);
    }
}
            ...
            <div class="card-body p-0">
                <ul class="list-group list-group-flush">
                    <?php foreach ($receivers as $receiver): ?>
                        <li class="list-group-item <?php echo ($selectedReceiver === $receiver['id']) ? 'active' : ''; ?>">
                            <a href="sender.php?receiver_id=<?php echo $receiver['id']; ?>" class="text-decoration-none d-block">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    $unreadStmt->execute([$_SESSION['user_id'], $receiver['id']]);
        ...
        <?php if ($selectedReceiver): ?>
            ...
            <div class="card shadow">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h4>Chat with <?php echo htmlspecialchars($receiverInfo['username']); ?></h4>
                </div>
        ...
        <?php else: ?>
            <div class="card shadow">
                <div class="card-body text-center py-5">
                    ...
                    <h4>Select a receiver to start chatting</h4>
                </div>
            </div>
        <?php endif; ?>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (messageForm) {
        messageForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                messageForm.reset();
                fetchNewMessages(); // Refresh chat after sending
            })
